  ...
  getCityDatas() {
    let cityDatas = wx.getStorage({
      ...
      success: (res) => {
        ...
        if (res.data.now.tmp >= 40) {
          this.setData({
            bcgImg: this.data.bcgImgList[0].src,
            bcgColor: this.data.bcgImgList[0].topColor,
            advice1: '天气酷热，建议穿单薄衣物，尽量停止户外活动。',
          })
          this.setNavigationBarColor()
        }
        else if (res.data.now.tmp >= 35) {
          this.setData({
            bcgImg: this.data.bcgImgList[1].src,
            bcgColor: this.data.bcgImgList[1].topColor,
            advice2: '天气闷热，适宜着丝麻、轻棉织物制作的短衣、短裙、薄短裙、短裤等夏季服饰。午后尽量减少户外活动。',
          })
          this.setNavigationBarColor()
        }
        else if (res.data.now.tmp >= 30) {
          this.setData({
            bcgImg: this.data.bcgImgList[1].src,
            bcgColor: this.data.bcgImgList[1].topColor,
            advice3: '天气炎热，适宜着短衫、短裙、薄型T恤衫、敞领短袖等夏季服饰。',
          })
          this.setNavigationBarColor()
        }
        else if (res.data.now.tmp >= 25) {
          this.setData({
            bcgImg: this.data.bcgImgList[1].src,
            bcgColor: this.data.bcgImgList[1].topColor,
            advice4: '天气较热，适宜着棉麻面料的短套装、长裙、T恤等夏季服饰。',
          })
          this.setNavigationBarColor()
        }
        else if (res.data.now.tmp >= 20) {
          this.setData({
            bcgImg: this.data.bcgImgList[1].src,
            bcgColor: this.data.bcgImgList[1].topColor,
            advice5: '天气温暖，适宜着单层棉麻的短套装、休闲服、薄长袖衬衫、单裤等。',
          })
          this.setNavigationBarColor()
        }
        else if (res.data.now.tmp >= 15) {
          this.setData({
            bcgImg: this.data.bcgImgList[1].src,
            bcgColor: this.data.bcgImgList[1].topColor,
            advice6: '天气温和，适宜着长裤、长袖T恤、夹衣、马甲衬衫、西服套装等春秋服饰。',
          })
          this.setNavigationBarColor()
        }
        else if (res.data.now.tmp >= 10) {
          this.setData({
            bcgImg: this.data.bcgImgList[1].src,
            bcgColor: this.data.bcgImgList[1].topColor,
            advice7: '天气较凉，适宜着一件羊毛衫、夹克衫、毛衣加薄外套、大衣、皮夹克等。',
          })
          this.setNavigationBarColor()
        }
        else if (res.data.now.tmp >= 5) {
          this.setData({
            bcgImg: this.data.bcgImgList[1].src,
            bcgColor: this.data.bcgImgList[1].topColor,
            advice8: '天气微冷，适宜着毛衣、风衣、大衣、毛套装、防寒服等。',
          })
          this.setNavigationBarColor()
        }
        else if (res.data.now.tmp >= 0) {
          this.setData({
            bcgImg: this.data.bcgImgList[1].src,
            bcgColor: this.data.bcgImgList[1].topColor,
            advice9: '天气冷，适宜着棉衣、羽绒衣、大衣、毛套装等冬季着装。',
          })
          this.setNavigationBarColor()
        }
        else if (res.data.now.tmp >= -5) {
          this.setData({
            bcgImg: this.data.bcgImgList[1].src,
            bcgColor: this.data.bcgImgList[1].topColor,
            advice10: '天气寒冷，适宜着棉衣、羽绒服，防寒大衣、冲锋装等。',
          })
          this.setNavigationBarColor()
        }
        else if (res.data.now.tmp >= -10) {
          this.setData({
            bcgImg: this.data.bcgImgList[1].src,
            bcgColor: this.data.bcgImgList[1].topColor,
            advice11: '天气极冷，适宜着厚棉衣、厚羽绒服、冬大衣、棉帽、棉手套等。',
          })
          this.setNavigationBarColor()
        }
        else {
  ...
  huanye: function () {
    let cityDatas = wx.getStorage({
      ...
      success: (res) => {
        ...
        if ((res.data.now.tmp >= 4) && (res.data.now.tmp <= 30) && (50 <= res.data.now.hum <= 60) && (res.data.now.wind_sc <= 4)) {
          wx.navigateTo({
            url: '/pages/running/running',
          })
        }
        else if ((res.data.now.tmp >= 10) && (res.data.now.tmp <= 13) && (50 <= res.data.now.hum <= 60) && (res.data.now.wind_sc <= 5)) {
          wx.navigateTo({
            url: '/pages/basketball/basketball',
          })
        }
        else if ((res.data.now.tmp >= 4) && (res.data.now.tmp <= 30) && (50 <= res.data.now.hum <= 60) && (res.data.now.wind_sc <= 4)) {
          wx.navigateTo({
            url: '/pages/football/football',
          })
        }
        else if ((res.data.now.tmp == 7) && (50 <= res.data.now.hum <= 60) && (res.data.now.wind_sc <= 4)) {
          wx.navigateTo({
            url: '/pages/badminton/badminton',
          })
        }
        else if ((res.data.now.tmp >= 13) && (res.data.now.tmp <= 16) && (50 <= res.data.now.hum <= 60) && (res.data.now.wind_sc <= 4)) {
          wx.navigateTo({
            url: '/pages/tennis/tennis',
          })
        }
        else if ((res.data.now.tmp >= 4) && (res.data.now.tmp <= 35) && (40 <= res.data.now.hum <= 60) && (res.data.now.wind_sc <= 5)) {
          wx.navigateTo({
            url: '/pages/suitable/suitable',
          })
        }
        else {
          wx.navigateTo({
            url: '/pages/unsuitable/unsuitable',
          })
        }
      }
    })
